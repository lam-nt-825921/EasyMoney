...
class PaymentRemoteDataSource @Inject constructor(
    ...
    suspend fun addPaymentCard(card: PaymentCard): Resource<Unit> =
        safeApiCall("Add card failed") { apiService.addPaymentCard(card) }

    suspend fun deletePaymentCard(cardId: String): Resource<Unit> =
        safeApiCall("Delete card failed") { apiService.deletePaymentCard(cardId) }

    suspend fun verifyCard(card: PaymentCard): Resource<Unit> =
        safeApiCall("Verify card failed") { apiService.verifyCard(card) }
    ...
    suspend fun topUp(amount: Long, cardId: String): Resource<Unit> =
        safeApiCall("Top up failed") { apiService.topUp(TopUpRequest(amount, cardId)) }

    suspend fun withdraw(amount: Long, cardId: String, biometricToken: String?): Resource<Unit> =
        safeApiCall("Withdraw failed") { apiService.withdraw(WithdrawRequest(amount, cardId, biometricToken)) }

    suspend fun toggleAutoDeduction(enabled: Boolean): Resource<Unit> =
        safeApiCall("Toggle auto deduction failed") { apiService.toggleAutoDeduction(AutoDeductionRequest(enabled)) }
