# Agent Tasks - 2026-06-02 Backend-Aligned Batch

Purpose: this is the only active frontend task list. Old tasks were removed to avoid multiple competing task sources.
...
Assume the implementation agent may only have the GitHub frontend repo. Backend source is not available to the frontend agent. Use the backend contract below as the expected integration boundary for this task batch. Public demo base URL remains `https://easymoney.lamgd.dev/`.

## Product Direction

Remove user-facing biometric/sinh trac hoc functionality from production UX. Biometric must not be required for withdraw, repayment, contract signing, or identity/profile completion.
...
Success remains `ApiResponse[dict]` or may become `ApiResponse[RepayResultDto]`.

Optional `RepayResultDto` fields:
...
Frontend requirement: after successful repay or early settlement, refresh rewards from `GET /api/v1/rewards/user` and update Home/Account/Rewards state when the user returns to those screens.
...
Money fields are JSON numbers. Parse as `Double` where current frontend money DTOs use `Double`, then round only for display.
...
```json
{
  ...
  "application_id": "APP_XXXXXXXX",
  "contract_id": "CONTRACT_APP_XXXXXXXX",
  "title": "Ho so vay da duoc duyet",
  "body": "Vui long kiem tra va ky hop dong de nhan giai ngan.",
...
```json
{
  ...
  "message": "So the khong hop le.",
  "field_errors": {
    "card_number": "So the phai co 16-19 chu so."
  }
}
```
...
- Remove or hide biometric/sinh trac hoc entries from security settings and identity/profile completion.
...
- Searching production UI code for `biometric`, `Sinh trac`, `sinh trac`, `Sinh trắc`, `sinh trắc` should not reveal active user-facing controls or required flows.
